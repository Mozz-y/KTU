    ...
    public class Register
    {
        ...
        public List<MusicQuestion> FindDifficultMusicQuestions()
        {
            ...
            var allQuestions = _organization1Questions
                ...
                // Handle logic when there is no III or II difficulty
                if (allQuestions.Any(mq => mq.Difficulty == "III"))
                {
                    return allQuestions.Where(mq => mq.Difficulty == "III").ToList();
                }
                else if (allQuestions.Any(mq => mq.Difficulty == "II"))
                {
                    return allQuestions.Where(mq => mq.Difficulty == "II").ToList();
                }
                else
                {
                    return allQuestions.Where(mq => mq.Difficulty == "I").ToList();
                }
        }
        ...
        public List<Question> FindAllDifficultQuestions()
        {
            // Combine all questions from all three organizations
            var allQuestions = _organization1Questions
                .Concat(_organization2Questions) // Combine Org1 and Org2 questions
